...
// Filter Panel Component - shows list with checkboxes
function FilterPanel({ 
  ...
  return (
    <div className="border-b border-zinc-800">
      <button
        onClick={onToggle}
        className={`w-full flex items-center justify-between px-3 py-2 text-left text-xs font-medium hover:bg-zinc-800/50 transition-colors ${
          selected.length > 0 ? "text-sky-400" : "text-zinc-400"
        }`}
      >
      ...
      {isOpen && (
        <div className="px-2 pb-2">
          ...
          <input
            ...
            className="w-full h-7 px-2 mb-2 bg-zinc-800 border border-zinc-700 rounded text-[11px] text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-600"
          />
          ...
          <div className="flex gap-2 mb-2">
            ...
            <button onClick={clearAll} className="text-[10px] text-zinc-400 hover:text-white">Clear</button>
          </div>
          
          {/* Options list */}
          <div className="max-h-40 overflow-y-auto space-y-0.5">
            {filtered.length === 0 ? (
              <div className="text-[11px] text-zinc-500 py-2">No results</div>
            ) : (
              filtered.map((o) => (
                <label
                  key={o}
                  className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer hover:bg-zinc-800 ${
                    selected.includes(o) ? "bg-sky-500/10" : ""
                  }`}
                >
                  <input
                    type="checkbox"
                    ...
                    onChange={() => toggle(o)}
                    className="w-3 h-3 rounded border-zinc-600 bg-zinc-800 text-sky-500 focus:ring-0 focus:ring-offset-0"
                  />
                  <span className={`text-[11px] truncate ${selected.includes(o) ? "text-sky-300" : "text-zinc-300"}`}>
                    {o}
                  </span>
                </label>
              ))
...
function ToggleFilter({ label, value, onChange }: { label: string; value: boolean | null; onChange: (v: boolean | null) => void }) {
  return (
    <button
      ...
      className={`h-6 px-2 rounded text-[10px] border transition-all whitespace-nowrap ${
...
function Tags({ items, color = "zinc", max = 2 }: { items: string[] | null; color?: string; max?: number }) {
  ...
  return (
    <div className="flex flex-wrap gap-0.5 items-center">
      {shown.map((t, i) => (
        <span key={i} className={`px-1 py-0.5 rounded border text-[9px] leading-tight ${colors[color]}`} title={t}>
...
function Socials({ p }: { p: Profile }) {
  return (
    <div className="flex items-center gap-0.5">
      {p.github_handle && <a href={`https://github.com/${p.github_handle}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-0.5 text-zinc-600 hover:text-white" title="GitHub"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg></a>}
      {p.twitter_handle && <a href={`https://twitter.com/${p.twitter_handle}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-0.5 text-zinc-600 hover:text-white" title="Twitter"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>}
      {p.telegram_handle && <a href={`https://t.me/${p.telegram_handle}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-0.5 text-zinc-600 hover:text-white" title="Telegram"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg></a>}
      {p.linkedin_handle && <a href={`https://linkedin.com/in/${p.linkedin_handle}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-0.5 text-zinc-600 hover:text-white" title="LinkedIn"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>}
      {p.profile_url && <a href={p.profile_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-0.5 text-zinc-600 hover:text-sky-400" title="Profile"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg></a>}
    </div>
  );
}
...
function ProfileRow({ p, expanded, onToggle }: { p: Profile; expanded: boolean; onToggle: () => void }) {
  return (
    <>
      <div
        onClick={onToggle}
        className={`grid grid-cols-[1fr_1.2fr_1fr_0.9fr_0.8fr_0.8fr_1fr_1fr_70px_50px] gap-1 px-2 py-1.5 border-b border-zinc-800/50 hover:bg-zinc-800/30 cursor-pointer items-center text-[11px] ${expanded ? "bg-zinc-800/40" : ""}`}
      >
        <div className="truncate text-zinc-400">{p.username || "—"}</div>
      ...
      {expanded && (
        <div className="bg-zinc-900/50 border-b border-zinc-800 px-3 py-3">
  ...
  return (
    <div className="border-b border-zinc-800/50">
      <div onClick={onToggle} className={`p-3 ${expanded ? "bg-zinc-800/40" : "hover:bg-zinc-800/30"} cursor-pointer`}>
        ...
        <div className="mb-2"><Tags items={p.tags} color="emerald" max={3} /></div>
...
export default function Home() {
    ...
    let result = profiles.filter((p) => {
      if (search) {
        ...
        const match = p.username?.toLowerCase().includes(s) || p.display_name?.toLowerCase().includes(s) || p.about?.toLowerCase().includes(s) ||
          ...
          p.looking_for_roles?.some((r) => r.toLowerCase().includes(s));
    ...
    <div className="h-full flex flex-col bg-zinc-900">
      ...
      <div className="p-3 border-b border-zinc-800">
        <div className="flex items-center justify-between mb-2">
          ...
          {filterCount > 0 && (
            <button onClick={clearAll} className="text-[10px] text-sky-400 hover:text-sky-300">
        ...
        <input
          ...
          className="w-full h-7 px-2 bg-zinc-800 border border-zinc-700 rounded text-[11px] text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-600"
        />
      </div>

      {/* Filter panels */}
      <div className="flex-1 overflow-y-auto">
        <FilterPanel title="Country" options={options.countries} selected={selCountries} onChange={setSelCountries} isOpen={openPanels.country || false} onToggle={() => togglePanel("country")} />
        <FilterPanel title="Language" options={options.languages} selected={selLanguages} onChange={setSelLanguages} isOpen={openPanels.language || false} onToggle={() => togglePanel("language")} />
        <FilterPanel title="Location" options={options.locations} selected={selLocations} onChange={setSelLocations} isOpen={openPanels.location || false} onToggle={() => togglePanel("location")} />
        <FilterPanel title="Role" options={options.roles} selected={selRoles} onChange={setSelRoles} isOpen={openPanels.role || false} onToggle={() => togglePanel("role")} />
        <FilterPanel title="Interest" options={options.topics} selected={selTopics} onChange={setSelTopics} isOpen={openPanels.topic || false} onToggle={() => togglePanel("topic")} />
        <FilterPanel title="Skills" options={options.tags} selected={selTags} onChange={setSelTags} isOpen={openPanels.tag || false} onToggle={() => togglePanel("tag")} />
        <FilterPanel title="Company" options={options.companies} selected={selCompanies} onChange={setSelCompanies} isOpen={openPanels.company || false} onToggle={() => togglePanel("company")} />
        <FilterPanel title="Looking For" options={options.lookingFor} selected={selLookingFor} onChange={setSelLookingFor} isOpen={openPanels.lookingFor || false} onToggle={() => togglePanel("lookingFor")} />
        <FilterPanel title="Account Role" options={options.accountRoles} selected={selAccountRoles} onChange={setSelAccountRoles} isOpen={openPanels.accountRole || false} onToggle={() => togglePanel("accountRole")} />
        <FilterPanel title="Batch" options={options.batches} selected={selBatches} onChange={setSelBatches} isOpen={openPanels.batch || false} onToggle={() => togglePanel("batch")} />
        ...
        <div className="p-3 border-t border-zinc-800">
          ...
          <div className="text-[10px] text-zinc-500 uppercase mt-3 mb-2">Socials</div>
          <div className="flex flex-wrap gap-1">
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 flex">
      {/* Desktop Sidebar */}
      <div className={`hidden lg:block ${sidebarOpen ? "w-64" : "w-0"} transition-all duration-300 overflow-hidden border-r border-zinc-800`}>
        <SidebarContent />
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-zinc-950/95 backdrop-blur border-b border-zinc-800">
          <div className="flex items-center gap-2 px-2 py-2">
            {/* Toggle sidebar */}
            <button
              onClick={() => { if (window.innerWidth >= 1024) setSidebarOpen(!sidebarOpen); else setMobileSidebar(true); }}
            ...
            <span className="text-sm font-semibold text-white">Colosseum</span>
            <span className="text-zinc-600 hidden sm:inline">|</span>
            <span className="text-[11px] text-zinc-400">{filtered.length.toLocaleString()} / {profiles.length.toLocaleString()}</span>
          ...
          <div className="hidden lg:grid grid-cols-[1fr_1.2fr_1fr_0.9fr_0.8fr_0.8fr_1fr_1fr_70px_50px] gap-1 px-2 py-1 bg-zinc-900/50 border-t border-zinc-800 text-[10px] font-medium text-zinc-500 uppercase">
              ...
              {filtered.slice(0, visibleCount).map((p) => (
                <div key={p.id}>
                  ...
                  <div className="hidden lg:block">
                    <ProfileRow p={p} expanded={expandedId === p.id} onToggle={() => setExpandedId(expandedId === p.id ? null : p.id)} />
                  </div>
                </div>
              ))}
