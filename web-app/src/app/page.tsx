...
function FilterPanel({ 
  ...
  return (
    <div className="border-b border-zinc-800">
      <button
        onClick={onToggle}
        className={`w-full flex items-center justify-between px-3 py-2.5 text-left text-xs font-medium hover:bg-zinc-800/50 transition-colors ${
      ...
      {isOpen && (
        <div className="px-3 pb-3">
          <input
            ...
            className="w-full h-8 px-2 mb-2 bg-zinc-800 border border-zinc-700 rounded text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-600"
          />
          ...
          <div className="max-h-44 overflow-y-auto space-y-0.5">
            {filtered.length === 0 ? (
              <div className="text-xs text-zinc-500 py-2">No results</div>
            ) : (
              filtered.map((o) => (
                <label key={o} className={`flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer hover:bg-zinc-800 ${selected.includes(o) ? "bg-sky-500/10" : ""}`}>
                  <input
                    ...
                    onChange={() => toggle(o)}
                    className="w-3.5 h-3.5 rounded border-zinc-600 bg-zinc-800 text-sky-500 focus:ring-0 focus:ring-offset-0 cursor-pointer"
                  />
...
function ToggleFilter({ label, value, onChange }: { label: string; value: boolean | null; onChange: (v: boolean | null) => void }) {
  return (
    <button
      ...
      className={`h-7 px-2.5 rounded text-xs border transition-all whitespace-nowrap ${
...
function Tags({ items, color = "zinc", max = 3 }: { items: string[] | null; color?: string; max?: number }) {
  ...
  const colors: Record<string, string> = {
    zinc: "bg-zinc-700/50 text-zinc-200 border-zinc-600",
  ...
  return (
    <div className="flex flex-wrap gap-1 items-center">
      {shown.map((t, i) => (
        <span key={i} className={`px-1.5 py-0.5 rounded border text-[11px] ${colors[color]}`} title={t}>
          {t.length > 14 ? t.slice(0, 14) + "…" : t}
        </span>
      ))}
      {!expanded && rest > 0 && (
        <button onClick={(e) => { e.stopPropagation(); setExpanded(true); }} className="px-1 text-[11px] text-sky-400 hover:text-sky-300">+{rest}</button>
      )}
      {expanded && items.length > max && (
        <button onClick={(e) => { e.stopPropagation(); setExpanded(false); }} className="px-1 text-[11px] text-zinc-400 hover:text-zinc-300">less</button>
      )}
    </div>
  );
}

// Social icons - fixed to show all including LinkedIn
    ...
    <div className="flex items-center gap-1 flex-wrap">
      {socials.map((s, i) => (
        <a key={i} href={s.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} 
          className="p-1 text-zinc-500 hover:text-white rounded hover:bg-zinc-700/50" title={s.title}>
      ...
      {p.profile_url && (
        <a href={p.profile_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} 
          className="p-1 text-zinc-500 hover:text-sky-400 rounded hover:bg-zinc-700/50" title="Profile">
...
// Profile Row - bigger text
function ProfileRow({ p, expanded, onToggle }: { p: Profile; expanded: boolean; onToggle: () => void }) {
  return (
    <>
      <div
        onClick={onToggle}
        className={`grid grid-cols-[140px_160px_140px_160px_90px_100px_180px_180px_70px_80px] gap-2 px-3 py-2 border-b border-zinc-800/50 hover:bg-zinc-800/30 cursor-pointer items-center text-xs ${expanded ? "bg-zinc-800/40" : ""}`}
      >
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [profiles, search, selCountries, selLocations, selLanguages, selRoles, selTopics, selTags, selCompanies, selLookingFor, lookingTeam, isStudent, hasGithub, hasTwitter, hasTelegram, hasLinkedin, sortField, sortOrder]);

  useEffect(() => { setVisibleCount(100); }, [search, selCountries, selLocations, selLanguages, selRoles, selTopics, selTags, selCompanies, selLookingFor, lookingTeam, isStudent, hasGithub, hasTwitter, hasTelegram, hasLinkedin]);
  ...
  const filterCount = selCountries.length + selLocations.length + selLanguages.length + selRoles.length + selTopics.length +
    selTags.length + selCompanies.length + selLookingFor.length +
  ...
  if (loading) return (
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-zinc-400 text-sm">Loading {loadingProgress.toLocaleString()} profiles...</p>
      </div>
    </div>
  );
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-zinc-950/95 backdrop-blur border-b border-zinc-800">
          <div className="flex items-center gap-3 px-3 py-2.5">
            ...
            <span className="text-sm text-zinc-400">{filtered.length.toLocaleString()} / {profiles.length.toLocaleString()}</span>
